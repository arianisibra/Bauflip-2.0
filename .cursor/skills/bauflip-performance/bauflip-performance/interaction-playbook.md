...
| | |
|---|---|
| **Problem** | 2 POSTs: `getProjectSheetHeadAction` then `getProjectSheetDetailsAction` |
| **HAR** | 2× `core` classified POSTs per sheet open |
| **Fix** | `getProjectSheetBootstrapAction` + `project_core_bootstrap` RPC |
| **Target** | **1×** `core` POST |
| **Status** | Code shipped — run `npm run db:push` then re-capture HAR |
