...
import { Calendar, Layers, Mail, ShieldAlert, UserPlus, Users } from "lucide-react";
        ...
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground">
            {isAdmin
              ...
              : "Zugriff auf Teamverwaltung ist nur für Administratoren möglich."}
          </p>
        </div>
      </div>

      {!isAdmin ? (
        <Card className="overflow-hidden border-amber-200/80 bg-gradient-to-br from-amber-50/90 to-amber-50/40 shadow-sm dark:border-amber-900/40 dark:from-amber-950/40 dark:to-transparent">
          <CardHeader className="flex flex-row items-start gap-3 pb-2">
            <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-amber-800 dark:bg-amber-950/80 dark:text-amber-200">
              <ShieldAlert className="size-5" />
            </div>
            <div className="min-w-0 space-y-1">
              <CardTitle className="text-base font-semibold tracking-tight">Kein Zugriff</CardTitle>
              <CardDescription className="text-sm leading-relaxed">
                Diese Seite ist für Administratoren reserviert. Ihre aktuelle Rolle:{" "}
                <span className="font-medium text-foreground">{roleLabel[role as keyof typeof roleLabel] ?? role}</span>.
                Bitte wenden Sie sich an eine Admin-Person in Ihrer Organisation.
              </CardDescription>
            </div>
          </CardHeader>
        </Card>
