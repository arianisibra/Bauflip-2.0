-- Phase Kal-DB: calendar range tasks in one roundtrip (appointments + project + technician).
...
as $$
  ...
  where a.starts_at >= p_range_start
    ...
    and (p_technician_id is null or a.assigned_technician_id = p_technician_id)
...
comment on function public.calendar_range_tasks_for_org is
  'Office/tech calendar: appointments in range with project + technician fields (RLS via security invoker).';
